...
var ViewModel1 = function() {
	...
	self.fetchCurrentActivityList=function() {
		...
		$.ajax({
				  type: "POST",
				  url: "/api/post/getallcertification",
				  dataType: "json",
				  data:{
				  	"token": self.token,
	...
	self.setPassActivity=function(data) {
		...
		$.ajax({
				  type: "POST",
				  url: "/api/post/setpasscertification",
				  dataType: "json",
				  data:{
				  	"token": self.token,
				  	"certlist":[data.id()]
	...
	self.setNoPassActivity=function(data) {
		...
		$.ajax({
				  type: "POST",
				  url: "/api/post/setnopasscertification",
				  dataType: "json",
				  data:{
				  	"token": self.token,
				  	"certlist":[data.id()]
...
var Activity = function (data) {
	this.checkresult=ko.observable(data.checkresult);
	this.id=ko.observable(data.id);
	...
	this.timestamp=ko.observable((new Date(Date.parse(data.timestamp))).toLocaleString()) ;
